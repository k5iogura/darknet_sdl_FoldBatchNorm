...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    clock_t time;
    int count = 0;
    float min_loss = 9999999999.;
    //while(i*imgs < N*120){
    while(get_current_batch(net) < net.max_batches){
        if(l.random && count++%10 == 0){
            printf("Resizing\n");
            //int dim = (rand() % 10 + 10) * 32;    //remove original
            //int dim = (rand() % 4 + 6) * 32;        //add
            int dim;
            int iw,ih;                      //add
            ih = (rand() % l.h/2 + l.h - 2);        //add
            iw = ih + 2;                        //add
            dim= iw * 32;                       //add
            args.w = dim;                       //add
            dim= ih * 32;
            ...
            printf("to W/H %d %d\n", args.w,args.h);
            //args.w = dim; //remove original
            //args.h = dim; //remove original
        ...
        */
        ...
        if(i>1000 && min_loss > avg_loss){
            min_loss = avg_loss;
#ifdef GPU
            if(ngpus != 1) sync_nets(nets, ngpus, 0);
#endif
            char buff[256];
            sprintf(buff, "%s/%s_minloss.weights", backup_directory, base);
            save_weights(net, buff);
        }
        free_data(train);
    }
...
void validate_detector_flip(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    ...
    //for(j = 0; j < l.w*l.h*l.n; ++j) probs[j] = calloc(classes+1, sizeof(float *));   //DEBUG
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        box *boxes = calloc(l.w*l.h*l.n, sizeof(box));
        float **probs = calloc(l.w*l.h*l.n, sizeof(float *));
        //for(j = 0; j < l.w*l.h*l.n; ++j) probs[j] = calloc(l.classes + 1, sizeof(float *));   //DEBUG
        for(j = 0; j < l.w*l.h*l.n; ++j) probs[j] = calloc(l.classes + 1, sizeof(float));
        float **masks = 0;
        if (l.coords > 4){
            masks = calloc(l.w*l.h*l.n, sizeof(float*));
            //for(j = 0; j < l.w*l.h*l.n; ++j) masks[j] = calloc(l.coords-4, sizeof(float *));  //DEBUG
